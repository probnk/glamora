...
class _MessagingScreenState extends State<MessagingScreen> {
    ...
    _scrollController.addListener(() {
      if (_scrollController.hasClients) {
        Provider.of<ChatProvider>(context, listen: false)
            .updateScrollButtonVisibility(_scrollController.position.pixels,
        ...
        ChangeNotifierProvider<ChatProvider>(
          create: (_) => ChatProvider()
            ..initializeChat(
                ...
                photoUrl: userProvider.pictureUrl),
        ),
          ...
          children: [
                ...
                if (!_isConnected && !provider.hasMessages) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'No internet connection. Please connect to view messages.',
                          style: TextStyle(
                              color: isDarkMode ? Colors.white : Colors.black),
                          textAlign: TextAlign.center,
                        ),
                ...
                return Column(
                  children: [
                    provider.isSelectionMode
                        ? _buildSelectionAppBar(provider, isDarkMode)
                    ...
                    if (provider.messagesRef != null)
                      Consumer<InputProvider>(
                        builder: (context, inputProvider, _) =>
                            _buildMessageInput(
                                provider, inputProvider, isDarkMode),
                      ),
                    if (provider.messagesRef == null)
            ...
            Consumer<ChatProvider>(
              builder: (context, chatProvider, _) {
                return chatProvider.showScrollToBottomButton
                    ? Positioned(
                  bottom: 80,
                  right: 16,
                  child: FloatingActionButton(
                    mini: true,
                    backgroundColor:
                    isDarkMode ? lightGreen : lightPurple,
  ...
  Widget _buildSelectionAppBar(ChatProvider provider, bool isDarkMode) {
    return AppBar(
      ...
      title: Text(
        ...
        style: GoogleFonts.exo2(color: white),
      ),
      actions: [
        if (provider.canEdit)
          IconButton(
            icon: const Icon(Icons.edit, color: white),
            onPressed: () => _showEditDialog(provider, isDarkMode),
          ),
        IconButton(
          icon: const Icon(Icons.delete, color: white),
          onPressed: () => _confirmDelete(provider, isDarkMode),
        ),
      ],
    );
  }
  ...
  void _showEditDialog(ChatProvider provider, bool isDarkMode) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: productTitle(
            text: "Edit Message", color: isDarkMode ? white : grayBlack),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              if (controller.text.trim().isNotEmpty) {
                await provider.updateMessage(
                  provider.selectedMessageIds.first,
            ...
            child:
            productTitle(text: "Save", color: isDarkMode ? green : purple),
          ),
        ],
      ),
    );
  ...
  void _confirmDelete(ChatProvider provider, bool isDarkMode) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: isDarkMode ? lightGrayBlack : white,
        title: productTitle(
            text: "Delete Message", color: isDarkMode ? white : grayBlack),
        content: smallFont(
  ...
  String _formatLastSeen(dynamic lastSeen) {
    ...
    try {
      if (lastSeen is int) {
        ...
        // Optional: in case saved as stringified epoch
        lastSeenTime = DateTime.fromMillisecondsSinceEpoch(int.parse(lastSeen));
      } else {
        return 'recently';
      }
    } catch (e) {
  ...
  Widget _buildMessageList(ChatProvider provider, bool isDarkMode) {
    if (provider.messagesRef == null) {
      return const Center(child: Text('Messages not initialized'));
    }

    if (!_isConnected || provider.error != null) {
      return FutureBuilder<List<MessageModel>>(
        future: provider.dbHelper.getMessages(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (messages.isEmpty) {
            return _buildEmptyMessageList(isDarkMode);
          }
          return ListView.builder(
                ...
                children: [
                  if (showDate)
                    Padding(
                      ...
                      child: Center(
                        child:
                        smallFont(text: date, color: Colors.grey.shade500),
                      ),
                    ),
                  _buildMessageItem(
                      message, isDarkMode, provider, time, isSent),
                ],
              );
            },
          );
  ...
  Widget _buildMessageItem(MessageModel message, bool isDarkMode,
      ChatProvider provider, String time, bool isSent) {
    final isSelected = provider.selectedMessageIds.contains(message.id);
    return GestureDetector(
      onTap: () {
        if (provider.isSelectionMode && isSent && message.id != null) {
      ...
      onHorizontalDragEnd: (details) {
        if (!provider.isSelectionMode && details.primaryVelocity! > 0) {
          provider.setRepliedMessage(
            message,
            message.isSender ? "Customer" : "Seller",
          );
        }
      },
      onLongPressStart: (details) {
        ...
        if (!message.isSender) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: smallFont(text: "You Can't react on Your Own Message")));
        }
      },
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: isDarkMode
                      ...
                      : (isSent ? Colors.deepPurple : Colors.grey.shade500),
                  width: 1,
                ),
              ),
              child: Column(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  smallFont(
                                      text: provider.repliedTo == "Customer"
                                          ? "You"
                                          : "Seller",
                                      color: isDarkMode ? white : grayBlack,
                                      weight: FontWeight.bold),
                                  _buildPreviewContent(message.repliedMessage,
                                      message.repliedType, isDarkMode),
                                ],
                              ),
                            ),
                          ],
                  ...
                  onTap: () {
                    if (message.isSender) {
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content: smallFont(
                              text:
                              "You can't able to remove your message Reaction")));
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          _buildPreviewContent(message.message,
                                              message.messageType, isDarkMode),
                                        ],
                                      ),
                                    ),
                                  ],
  ...
  Widget _buildPreviewContent(String content, String type, bool isDarkMode) {
    switch (type) {
      case 'image':
        return ClipRRect(
          ...
          child: Image.network(
            ...
            height: 40,
            fit: BoxFit.cover,
            errorBuilder: (context, error, stackTrace) => smallFont(
              text: 'Image',
              color: isDarkMode ? white : grayBlack,
            ),
          ),
      ...
      case 'voice':
        return Row(
          children: [
            Icon(Icons.mic, size: 16, color: isDarkMode ? white : grayBlack),
            smallFont(
              text: 'Voice message',
              color: isDarkMode ? white : grayBlack,
            ),
  ...
  Widget _buildMessageContent(
      MessageModel message, bool isDarkMode, ChatProvider provider) {
    if (message.status == 'uploading' ||
      ...
      if (message.messageType == 'image') {
        ...
        return VoiceBubble(
          audioPath: message.localPath ?? message.message,
          localPath: message.localPath,
          isPlaying: provider.isPlayingMessage(message.id),
          onPlayPause: () => provider.togglePlay(
              message.id!, message.localPath ?? message.message),
        );
      }
    }
        ...
        return ClipRRect(
          ...
          child: CachedNetworkImage(
            ...
            placeholder: (context, url) => Center(
              child: reusableShimmerContainer(
                  context: context, isDarkMode: isDarkMode, height: 200),
            ),
  ...
  Widget _buildMessageInput(ChatProvider chatProvider,
      InputProvider inputProvider, bool isDarkMode) {
    return Container(
      padding: const EdgeInsets.all(8),
      color: isDarkMode ? lightGrayBlack : white,
      child: Column(
        children: [
          if (chatProvider.repliedMessage.isNotEmpty &&
              chatProvider.repliedTo.isNotEmpty)
            Container(
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            smallFont(
                              text: chatProvider.repliedTo == "Customer"
                                  ? "Replying Yourself"
                                  : "Replying Seller",
                              color: isDarkMode
                                  ? Colors.greenAccent
                            ...
                            _buildPreviewContent(chatProvider.repliedMessage,
                                chatProvider.repliedType, isDarkMode),
                          ],
                        ),
                      ),
                    ),
          ...
          Row(
            children: _buildInputRow(chatProvider, inputProvider, isDarkMode),
          ),
        ],
      ),
    );
  ...
  List<Widget> _buildInputRow(ChatProvider chatProvider,
      InputProvider inputProvider, bool isDarkMode) {
    List<Widget> children = [
      if (inputProvider.controller.text.isEmpty)
        IconButton(
          icon: Icon(Icons.attach_file, color: isDarkMode ? white : grayBlack),
          onPressed: inputProvider.isRecording ||
              inputProvider.pendingImage != null ||
              inputProvider.pendingVoicePath != null
              ? null
      ...
      if (inputProvider.controller.text.isEmpty)
        IconButton(
          icon: Icon(
            inputProvider.isRecording ? Icons.stop : Icons.mic,
            color: isDarkMode ? white : grayBlack,
          ),
          onPressed: inputProvider.pendingImage != null ||
              inputProvider.pendingVoicePath != null
              ? null
              : (inputProvider.isRecording
    ...
    if (inputProvider.isRecording) {
      inputWidget = Row(
        children: [
          IconButton(
            ...
            onPressed: inputProvider.cancelRecording,
          ),
          Expanded(
          ...
          smallFont(
            text: '${inputProvider.recordingDuration.inSeconds}s',
            color: isDarkMode ? white : grayBlack,
          ),
        ],
      );
      ...
      inputWidget = Row(
        children: [
          Image.file(inputProvider.pendingImage!,
              width: 60, height: 60, fit: BoxFit.cover),
          const Spacer(),
          IconButton(
            icon: const Icon(Icons.close),
            onPressed: inputProvider.cancelPendingImage,
          ),
        ],
      );
      ...
      inputWidget = Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: inputProvider.cancelPendingVoice,
          ),
        ],
      );
    } else {
    ...
    children.add(
      InkWell(
        ...
        child: CircleAvatar(
          radius: 25,
          backgroundColor: isDarkMode ? grayBlack : lightPurple,
